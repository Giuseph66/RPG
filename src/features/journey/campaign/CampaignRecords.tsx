...
export function CampaignRecords({ quests = [], npcs = [], objectives = [], onIntent, sections = ["objectives", "quests", "npcs"] }: CampaignRecordsProps) {
  ...
  return <div className={styles.records} data-layout={layout}>
    ...
    {showNpcs ? <section aria-labelledby="campaign-npcs-title"><div className={styles.sectionHeading}><span className={styles.sectionIcon} aria-hidden="true"><UsersThree size={20} weight="duotone" /></span><h2 id="campaign-npcs-title">NPCs</h2><span>{npcs.length}</span></div>{npcs.length === 0 ? <p className={styles.muted}>Nenhum NPC registrado.</p> : <ul className={styles.recordList}>{npcs.map((npc) => <li key={String(npc.id)}><div><strong>{npc.name}</strong><p>{npc.description}</p><span className={styles.recordMeta}>{npc.linkedEntityIds.length} vínculo(s)</span></div></li>)}</ul>}</section> : null}
  </div>;
}
