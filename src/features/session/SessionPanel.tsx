...
import { AppModal, Button, InlineStatus, Input, SectionCard } from "@components/ui";
...
export function SessionPanel({ session, authSession, campaignId, characters = [], npcs = [], syncState = "local" }: SessionPanelProps) {
  ...
  async function create() {
    ...
    setBusy(false);
    if (!result.ok) setMessage(result.error.message); else { setNumber(""); setTitle(""); setCreateOpen(false); setMessage("Sessão criada localmente e aguardando sincronização."); await load(); }
  }
  ...
  return <section className={styles.panel} aria-labelledby="session-title">
    <header className={styles.pageHeading}><div><p className={styles.eyebrow}>REGISTRO DA MESA</p><h1 id="session-title" className={styles.title} tabIndex={-1}>Sessões</h1><p className={styles.intro}>Crie, acompanhe e encerre sessões sem depender da rede. {authSession ? "A sessão usa sua conta autenticada." : "A sessão usa sua identidade local neste dispositivo; ela não será enviada sem uma conta autenticada."} {syncState === "offline" ? "As mudanças serão enviadas quando voltar online." : "O registro local é preservado enquanto a sincronização estiver pendente."}</p></div><Button onClick={() => setCreateOpen(true)}><Plus size={17} aria-hidden="true" /> Nova sessão</Button></header>
    ...
    <AppModal open={createOpen} title="Nova sessão" onClose={() => setCreateOpen(false)}><form className={styles.createForm} onSubmit={(event) => { event.preventDefault(); void create(); }}><Input label="Número" type="number" min={1} value={number} onChange={(event) => setNumber(event.target.value)} required /><Input label="Título (opcional)" value={title} onChange={(event) => setTitle(event.target.value)} maxLength={120} /><div className={styles.createActions}><Button type="button" variant="ghost" onClick={() => setCreateOpen(false)}>Cancelar</Button><Button type="submit" busy={busy} disabled={busy}>Criar sessão</Button></div></form></AppModal>
