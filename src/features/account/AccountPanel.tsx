...
function syncCopy(state: AccountPanelProps["syncState"]): { readonly label: string; readonly description: string; readonly tone: "info" | "warning" | "success" } {
  switch (state) {
    ...
    case "error": return { label: "Não sincronizado", description: "O Firebase não confirmou as alterações. Verifique o console do navegador e tente novamente.", tone: "warning" };
...
export function AccountPanel({ auth, availability, onBackToLocal, membership, syncState = "local", campaigns = [], onOpenCollaboration, onOpenSession, onOpenSettings, onPublishLocalCharacters }: AccountPanelProps) {
  ...
  async function saveProfile() {
    ...
    const published = onPublishLocalCharacters ? await onPublishLocalCharacters() : undefined;
    setBusy(false);
    if (published && !published.ok) {
      setProfileMessage(published.error.message);
      return;
    }
    const count = published?.value ?? 0;
    setProfileMessage(count > 0
      ? `Perfil salvo. ${count} ficha${count === 1 ? "" : "s"} local${count === 1 ? "" : "is"} entrou na fila de sincronização.`
      : "Perfil salvo neste dispositivo e aguardando sincronização.");
  }
  ...
  return (
    <section className={styles.panel} aria-labelledby="account-title">
      ...
      <div className={styles.dashboardGrid}>
            ...
            <SectionCard heading={mode === "sign-in" ? "Entrar" : "Criar conta"} headingLevel={2}>
              <form className={styles.form} onSubmit={(event) => void submit(event)} noValidate>
                ...
                <div className={styles.passwordField}>
                  <Input className={styles.passwordInput} label="Senha" type={passwordVisible ? "text" : "password"} autoComplete={mode === "register" ? "new-password" : "current-password"} value={password} onChange={(event) => setPassword(event.target.value)} required error={formError?.includes("senha") ? formError : undefined} />
                  <button
        ...
        <aside className={styles.secondaryColumn} aria-label="Apoio da conta">
